...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/18 12:05:49 by lgerard           #+#    #+#             */
/*   Updated: 2025/07/18 12:05:50 by lgerard          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pars_cam(t_app *app)
{
	...
	app->scene.cam_set = 1;
	if (!app->token[1] || !app->token[2] || !app->token[3] || app->token[4])
...
void	pars_light(t_app *app, t_light **lights)
{
	...
	if (app->scene.light_set)
		free_exit(app, "more than one light in the scene", 1);
	app->scene.light_set = 1;
