...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/18 12:01:36 by lgerard           #+#    #+#             */
/*   Updated: 2025/07/18 12:01:36 by lgerard          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pars_line(t_app *app, char *line)
{
	...
	if (!app->token || !app->token[0])
		return ;
	if (ft_strncmp(app->token[0], "fisheye", 8) == 0)
		app->fish = 1;
	else if (ft_strncmp(app->token[0], "A", 2) == 0)
		pars_amb(app);
...
"fisheye" : en debut de fichier indique un mode comme son nom l'indique
*/
...
void	pars_scene(t_app *app, char **av)
{
	...
	ft_printf("%s", &msg[0]);
	app->str = readfile(app, app->str, app->fd);
	close (app->fd);
	app->fd = -1;
	if (app->str[0] == 0)
